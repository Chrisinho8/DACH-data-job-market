"""Curated skill dictionary for German data-job postings.

Deliberately NOT an LLM:
  1. reproducibility - identical results on every run
  2. defensibility   - every match points at a specific string
  3. cost            - no API quota consumed

Aliases are regexes. The matcher sorts them longest-first and blanks
each matched span, so "sql" cannot re-match inside "postgresql".
"""
