...
# MAGIC %md
# MAGIC # 04 - Skill extraction (supporting section, not the headline)
# MAGIC
...
# MAGIC %md ## Tests - run these before trusting a single number
...
# MAGIC %md
# MAGIC ## The manual validation
# MAGIC
# MAGIC Export 100 random postings, label them by hand in a spreadsheet,
# MAGIC upload the labels back, and measure. Two hours, and it is the single
# MAGIC most credible thing in the whole repo.

# COMMAND ----------

sample = (p.select("posting_id", "title_raw", "description")
           .orderBy(F.rand(seed=42)).limit(100))

display(sample)     # download as CSV, add a skills_true column
                    # with semicolon-separated skill keys

# COMMAND ----------

# after uploading tests/labelled_sample.csv back to the volume
LABELS = f"/Volumes/{CAT}/bronze/raw/labelled_sample.csv"

try:
    lab = (spark.read.option("header", True)
                .csv(LABELS).toPandas())

    tp = fp = fn = 0
    for _, row in lab.iterrows():
        truth = {x.strip() for x in
                 str(row["skills_true"]).split(";") if x.strip()}
        pred = set(extract(f"{row['title_raw']} . "
                           f"{row['description']}"))
        tp += len(truth & pred)
        fp += len(pred - truth)
        fn += len(truth - pred)
    ...
    print("\nPut BOTH numbers in the README and the post.")
except Exception as e:
    print("labelled sample not uploaded yet:", e)
